...
- **`/api/strategy/executor-pnl`** and **`/api/strategy/true-pnl`** — **No** client `go_live_at`. A precursor query loads **`metrics_from` and `go_live_at`** from `candlelab_strategies_live` (`WHERE strategy_name = %s AND closed_at IS NULL ORDER BY id DESC LIMIT 1`). The anchor timestamp is **`metrics_from` if non-NULL, else `go_live_at`** from the same row. Executor requires **`strategy_name`** (replacing the previous `go_live_at` requirement).
...
- **Templates and other routes** — unchanged in this phase (clients may still send legacy fields; the server ignores them for these three endpoints).
...
- Strategy cards must send **`strategy_name`** for executor PnL and rely on DB state for anchors; orphaned requests without a matching open live row receive **400**.
