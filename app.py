...
def _build_trades_detail_rows(
    ...
    def _mk(ts):
        tu = to_utc_timestamp(ts)
        if tu is None:
            return None
        try:
            return tu.strftime("%Y-%m-%d %H:%M")
        except Exception:
            return None
...
def strategy_trades(strategy_id):
    ...
    return render_template(
        ...
        tp_mult=float(row.get("tp_mult") if row.get("tp_mult") is not None else 3.0),
        timeout=int(row.get("timeout") if row.get("timeout") is not None else 20),
